...
class JSONDataSourcePlugin(DataSourcePlugin):
    ...
    def get_raw_data_by_entity(self, name: str, entity_id: str) -> tuple[bytes, str]:
        obj = self._produce_json_data_obj(name)
        builder = JSONDataSourcePlugin._create_schema_builder_from_json_object(obj)
        schema = builder.to_schema()
        json_path_lookup = find_json_paths_for_schema(schema)
        result = JSONPath(json_path_lookup[entity_id]).parse(obj)
        return json.dumps(result).encode('utf-8'), 'application/json'


def find_json_paths_for_schema(schema: dict[str, Any]) -> dict[str, str]:
    json_path_per_name = {}
    existing_names = set()

    def make_class_name(field_name: str | None, parent_type: str) -> str:
        type_suffix = '' if parent_type == 'object' else 'Item'
        if field_name is None:
            field_name = 'model'
        counter = 1
        new_name = f'{field_name[0].upper()}{field_name[1:]}{type_suffix}'
        while new_name in existing_names:
            new_name = f'{field_name[0].upper()}{field_name[1:]}{counter}{type_suffix}'
            counter += 1
        existing_names.add(new_name)
        return new_name

    def traverse(node: dict[str, Any], field_name: str | None, parent_path: str | None, parent_type: str):
        if isinstance(node, dict) and 'type' in node:
            node_type = node['type']
            if parent_path is None and field_name is None:
                current_path = '$'
            else:
                if parent_type == 'object':
                    current_path = f'{parent_path}.{field_name}'
                else:
                    current_path = parent_path
            if node_type == 'object':
                # Perform insertion before traversing, so the numbered elements are further nested
                class_name = make_class_name(field_name, parent_type)
                json_path_per_name[class_name] = current_path

                if 'properties' in node:
                    props = node['properties']
                    for key, child in props.items():
                        traverse(child, key, current_path, 'object')

            if node_type == 'array':
                current_path += '[*]'
                if 'items' in node:
                    traverse(node['items'], field_name, current_path, 'array')

    traverse(schema, None, None, 'object')
    return json_path_per_name
...
#     json_path_per_name
#
#     def traverse(node: dict[str, Any], parent_key=None):
#
#         if 'type' in node:
#             node_type = node['type']
#             if node_type == 'object':
...
#                     for key, child in prop_items:
#                         traverse(child, key)
#
#
#         keys_by_type = collections.defaultdict(list)
#         for key, value in node.items():
#             keys_by_type[type(value)].append(key)
#
#         for key in keys_by_type[dict]:
#
#
#
# def extract_definitions(schema):
#     # this function was generated by copilot and it seems it is buggy
#     defs = {}
#     # counter = 1
#
...
#                     def_name = f'{parent}Item'
#                     defs[def_name] = node
#                     return {"$ref": f"#/$defs/{def_name}"}
#                 elif key == "items" and isinstance(value, dict) and value.get("type") == "object":
#                     def_name = f"{parent}Item"
#                     defs[def_name] = value
#                     node[key] = {"$ref": f"#/$defs/{def_name}"}
#                 else:
#                     node[key] = traverse(value, key)
#         elif isinstance(node, list):
#             for i in range(len(node)):
#                 node[i] = traverse(node[i], parent)
#         return node
#
