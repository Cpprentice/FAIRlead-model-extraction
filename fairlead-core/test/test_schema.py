...
from fairlead_core.schema import get_precedence_keys, merge_schema_dicts, merge_schema_lists
...
def test_merge_schema_dicts_simple_list(mocker):
    mocker.patch('fairlead_core.schema.merge_schema_lists', Mock(side_effect=lambda a, b: b))
    result = merge_schema_dicts({
        'key1': 123,
        'key2': [123]
    }, {
...
def test_merge_schema_lists_non_named_list(mocker):
    mocker.patch('fairlead_core.schema.get_name_key', Mock(return_value=None))
    result = merge_schema_lists([123], [456])
    assert result == [456]


def test_merge_schema_lists_named_list_independent(mocker):
    mocker.patch('fairlead_core.schema.get_name_key', Mock(return_value='name'))
    mocker.patch('fairlead_core.schema.get_precedence_keys_from_dicts', Mock(return_value=[('abc',), ('def',)]))
...
def test_merge_schema_lists_named_list_merging(mocker):
    mocker.patch('fairlead_core.schema.get_name_key', Mock(return_value='name'))
    mocker.patch('fairlead_core.schema.get_precedence_keys_from_dicts', Mock(return_value=[('abc', 'def')]))
    mocker.patch('fairlead_core.schema.merge_schema_dicts', Mock(return_value={'name': ['def', 'abc']}))
    result = merge_schema_lists([
        {
            'name': ['abc']
        }
