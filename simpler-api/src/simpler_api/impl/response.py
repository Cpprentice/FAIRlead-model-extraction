...
def wrap_response_according_to_request(request: Request, response: Any) -> Response:
    if 'accept' not in request.headers:
        return response
    requested_data_type = request.headers['accept']
    return wrap_response_according_to_accept_header(requested_data_type, response,)


def wrap_response_according_to_accept_header(requested_data_type: str, response: Any) -> Response:
